...
dnl Output: If qt version is auto, set emrals_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$emrals_qt_want_version" = xauto && test "x$emrals_qt_force" != xyes; then
    if test "x$emrals_enable_qt" != xno; then
      AC_MSG_WARN([$1; emrals-qt frontend will not be built])
    fi
    emrals_enable_qt=no
    emrals_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "x$emrals_enable_qt" != xno && test "x$emrals_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build emrals-qt GUI (default=auto)])],
    [
     emrals_qt_want_version=$withval
     if test "x$emrals_qt_want_version" = xyes; then
       emrals_qt_force=yes
       emrals_qt_want_version=auto
     fi
    ],
    [emrals_qt_want_version=auto])
...
dnl Outputs: emrals_enable_qt, emrals_enable_qt_dbus, emrals_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "x$emrals_cv_static_qt" = xyes; then
  ...
  BITCOIN_QT_CHECK([
    emrals_enable_qt=yes
    emrals_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      emrals_enable_qt_test=no
    fi
    emrals_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      emrals_enable_qt_dbus=yes
    fi
  ...
  AC_MSG_RESULT([$emrals_enable_qt (Qt5)])
...
dnl Output: emrals_cv_qt5=yes|no
AC_DEFUN([_BITCOIN_QT_CHECK_QT5],[
  AC_CACHE_CHECK(for Qt 5, emrals_cv_qt5,[
  AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
      #include <QtCore/qconfig.h>
      #ifndef QT_VERSION
      #  include <QtCore/qglobal.h>
...
AC_DEFUN([_BITCOIN_QT_CHECK_QT58],[
  AC_CACHE_CHECK(for > Qt 5.7, emrals_cv_qt58,[
  AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
      #include <QtCore/qconfig.h>
      #ifndef QT_VERSION
      #  include <QtCore/qglobal.h>
...
dnl Output: emrals_cv_static_qt=yes|no
dnl Output: Defines QT_STATICPLUGIN if plugins are static.
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, emrals_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
      ...
      [emrals_cv_static_qt=yes],
      [emrals_cv_static_qt=no])
    ])
  if test "x$emrals_cv_static_qt" = xyes; then
...
AC_DEFUN([_BITCOIN_QT_FIND_STATIC_PLUGINS],[
    if test "x$qt_plugin_path" != x; then
     ...
     m4_ifdef([PKG_CHECK_MODULES],[
       if test x$emrals_cv_qt58 = xno; then
       ...
       if test "x$TARGET_OS" = xwindows; then
         AC_CACHE_CHECK(for Qt >= 5.6, emrals_cv_need_platformsupport,[
           AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
               #include <QtCore/qconfig.h>
               #ifndef QT_VERSION
               #  include <QtCore/qglobal.h>
           ...
           [emrals_cv_need_platformsupport=yes],
           [emrals_cv_need_platformsupport=no])
         ])
         if test "x$emrals_cv_need_platformsupport" = xyes; then
           if test x$emrals_cv_qt58 = xno; then
...
dnl Inputs: emrals_qt_want_version (from --with-gui=). The version to check
dnl         first.
dnl Inputs: $1: If emrals_qt_want_version is "auto", check for this version
dnl         first.
...
dnl Inputs: emrals_qt_want_version (from --with-gui=). The version to use.
...
AC_DEFUN([_BITCOIN_QT_FIND_LIBS_WITHOUT_PKGCONFIG],[
  ...
  BITCOIN_QT_CHECK([
    if test "x$emrals_qt_want_version" = xauto; then

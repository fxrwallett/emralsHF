 ...
 */
static const char* EMRALS_PID_FILENAME = "emralsd.pid";

static fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", EMRALS_PID_FILENAME)));
}
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", EMRALS_CONF_FILENAME), false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", EMRALS_PID_FILENAME), false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-litemode", strprintf("Disable all EMRALS specific functionality (Masternodes, Governance) (default: %u)", false), false, OptionsCategory::OPTIONS);
    gArgs.AddArg("-sporkaddr=<emralsaddress>", "Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.", false, OptionsCategory::OPTIONS);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/emrals/EMRALS>";
    const std::string URL_WEBSITE = "<https://www.emrals.com/>";
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", EMRALS_CONF_FILENAME));
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if emrals is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if emrals is started while in a temporary directory.\n",
    ...
    if (fLiteMode)
        InitWarning(_("You are starting in lite mode, all EMRALS-specific functionality is disabled.").translated);
    ...
    // ********************************************************* Step 10-C: schedule EMRALS-specific tasks

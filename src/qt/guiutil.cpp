...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a EMRALS address (e.g. %1)").arg(
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("emrals:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", EMRALS_CONF_FILENAME));
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "EMRALS (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("EMRALS (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("emrals-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            ...
            optionFile << strprintf("Name=EMRALS (%s)\n", chain);

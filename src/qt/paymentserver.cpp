...
const int EMRALS_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString EMRALS_IPC_PREFIX("emrals:");
#ifdef ENABLE_BIP70
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(EMRALS_IPC_PREFIX, Qt::CaseInsensitive)) // emrals: URI
        {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(EMRALS_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(EMRALS_IPC_CONNECT_TIMEOUT);
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("emrals://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'emrals://' is not a valid URI. Use 'emrals:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(EMRALS_IPC_PREFIX, Qt::CaseInsensitive)) // emrals: URI
    {

...
RPCArg GetHelpArg(std::string strParamName)
{
    static const std::map<std::string, RPCArg> mapParamHelp = {
        {"collateralAddress",
            {"collateralAddress", RPCArg::Type::STR, RPCArg::Optional::NO, "The emrals address to send the collateral to"},
        },
        {"collateralHash",
        ...
        {"ownerAddress",
            {"ownerAddress", RPCArg::Type::STR, RPCArg::Optional::NO, "The emrals address to use for payee updates and proposal voting.\n"
        ...
        {"payoutAddress",
            {"payoutAddress", RPCArg::Type::STR, RPCArg::Optional::NO, "The emrals address to use for masternode reward payments."}
        },
        {"proTxHash",
...
// Allows to specify EMRALS address or priv key. In case of EMRALS address, the priv key is taken from the wallet
...
void protx_register_fund_help(const CWallet* pwallet)
{
    throw std::runtime_error(
        RPCHelpMan{"protx register_fund",
            "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 2500 EMRALS\n"
...
UniValue protx_register(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > paramIdx + 6) {
        ...
        if (!IsValidDestination(fundAddress))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid EMRALS address: ") + request.params[paramIdx + 6].get_str());
    }
...
UniValue protx_update_service(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() >= 6) {
        ...
        if (!IsValidDestination(feeSourceAddress))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid EMRALS address: ") + request.params[5].get_str());
...
UniValue protx_revoke(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > 4) {
        ...
        if (!IsValidDestination(feeSourceAddress))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid EMRALS address: ") + request.params[4].get_str());
        FundSpecialTx(pwallet, tx, ptx, feeSourceAddress);
    // TODO:

...
const char * const EMRALS_CONF_FILENAME = "emrals.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "emralsd -foo=bar
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EMRALS";
#else
    // Unix
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", EMRALS_CONF_FILENAME);
